...
def set_device_params(ser, params):
    ...
    print(([0x16, 0x55] +
                          [utils.get_mode_bytes(params['mode'])] +
                          list(params.values())[0:-3])+ list(params.values())[-2:])
    packet = struct.pack('<BBHHHHHHHHHHHHdd',
                        *([0x16, 0x55] +
                          [utils.get_mode_bytes(params['mode'])] +
                          list(params.values())[0:-3])+ list(params.values())[-2:])
    print(len(packet))
    ...
    #         expected_echo += struct.pack('!d', params[param])
    #     else:
    #         expected_echo += b'\x00' + utils.get_mode_bytes(params[param])

    ser.open()
    ser.write(packet)
    echoed_params = ser.read(20)
    ser.close()
    print(echoed_params)
